...
const audit = { schemaVersion: 1 as const, assetId: id, revision: 2, submissionChecksum: 'a'.repeat(64), metadataChecksum: 'b'.repeat(64), rulesetId: 'adobe-stock-2026-09-08-v1', rulesetVersion: 'adobe-stock-2026-09-08-v1', findings: [], createdAt: '2026-09-13T00:00:00.000Z' }
describe('durable audit store', () => {
  ...
  it('runs final evidence verification before creating the audit sidecar', async () => {
    const root = node(); root.files.set('gandiwa-project.json', { content: '{}' }); root.dirs.set('reports', node())
    await expect(saveDurableAudit({ directory: directory(root), manifestSnapshot: '{}', audit, verifyCurrentEvidence: async () => { throw new Error('evidence changed externally') } })).rejects.toThrow(/evidence changed externally/)
    expect(root.dirs.get('reports')!.dirs.get('audits')?.files.has(`${id}-r2.json`) ?? false).toBe(false)
  })
})
