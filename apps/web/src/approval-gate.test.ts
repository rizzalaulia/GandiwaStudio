...
describe('approval gate', () => {
  it('requires explicit human approval after current evidence passes', () => expect(evaluateApprovalGate(base()).status).toBe('READY FOR HUMAN APPROVAL'))
  it('returns Adobe-ready only for an exact binding', () => {
  ...
  it('blocks missing audit and invalid metadata', () => {
    ...
    expect(evaluateApprovalGate({ ...base(), metadataValid: false }).status).toBe('NOT READY')
  })
  it('blocks a blocking finding', () => expect(evaluateApprovalGate({ ...base(), audit: { ...audit, findings: [{ ruleId: 'fail', verdict: 'FAIL', message: 'bad', evidence: {} }] } }).status).toBe('STALE / BLOCKED'))
})
