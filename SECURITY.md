...
- Provider credentials are typed into the browser only through Issue #26's Setelan form over a same-origin HTTPS session; the backend stores them encrypted-at-rest (symmetric cipher keyed from `SESSION_SECRET`) and the raw key never reappears in any response or log. The browser itself never persists key material — no localStorage, no project files. Secure sessions are backend-owned. Tailscale and CORS do not replace application authentication or CSRF protection.
